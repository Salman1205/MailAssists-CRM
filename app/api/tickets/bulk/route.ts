    ...
    for (const ticketId of ticketIds) {
      try {
        ...
        if (status && ['open', 'pending', 'on_hold', 'closed'].includes(status)) {
          updatedTicket = await updateTicketStatus(ticketId, status, null);
        }

        // Update assignee if provided
        if (assigneeUserId !== undefined) {
          const assigneeId = assigneeUserId === null || assigneeUserId === '' ? null : assigneeUserId;
          updatedTicket = await assignTicket(ticketId, assigneeId, userId);
        }

        // Update tags if provided
        if (tags !== undefined && Array.isArray(tags)) {
          updatedTicket = await updateTicketTags(ticketId, tags, null);
        }
