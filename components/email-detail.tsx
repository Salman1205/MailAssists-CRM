...
// Helper function to sanitize HTML and remove potentially dangerous elements
// Also improves dark mode readability
const sanitizeEmailHtml = (html: string) => {
  ...
  const scripts = tmp.querySelectorAll("script, iframe, style")
  ...
  allElements.forEach(el => {
    ...
    // Remove inline colors that might cause contrast issues
...
const isHtmlContent = (content: string) => {
  ...
  return /<[^>]*(div|p|span|html|table|tr|td|br|img|a|b|i|em|strong|blockquote|pre|code|ul|ol|li|h[1-6]|body|font|style|meta|script)[\s>]/i.test(content)
}

// Detect potential attachments mentioned in email body
const detectAttachmentReferences = (content: string) => {
  if (!content) return []
  // Look for common attachment patterns: filename.ext, [attachment], "attached"
  const patterns = [
    /(?:attachment|attached|file|enclosed|see attached|document)[\s:]*([a-zA-Z0-9._\-]+\.[a-zA-Z0-9]{2,4})/gi,
    /\b([a-zA-Z0-9._\-]+\.(pdf|doc|docx|xlsx|xls|jpg|jpeg|png|zip|tar|rar))\b/gi,
  ]
...
interface EmailDetailProps {
  ...
  onBack?: () => void
  onToggleCustomer?: (email: string) => void
...
export default function EmailDetail({ emailId, onDraftGenerated, onBack, initialEmailData, onToggleCustomer, showCustomerPanel, ticketId, hideCloseButton }: EmailDetailProps) {
  ...
  const fetchThread = async () => {
    try {
      ...
      // Fetch email from CRM API
      // Explicitly include cookies for session auth and avoid caching
      const emailResponse = await fetch(`/api/crm/emails/${emailId}`, {
        credentials: 'include',
        cache: 'no-cache',
      ...
      if (emailResponse.ok) {
        try {
          ...
          // For CRM emails, there's no thread - just single messages
        ...
        // If we already have preview content, don't surface a hard error
        if (!initialEmailData) {
          setError("Failed to load email")
        } else {
          // Keep showing the preview without an error banner
          setError(null)
        }
      }
    } catch (err) {
  ...
  const handleGenerateDraft = async () => {
    ...
    try {
      setGenerating(true)
      setError(null)
      const response = await fetch(`/api/crm/emails/${emailId}/draft`, {
        method: 'POST'
      })
  ...
  const handleRegenerate = async () => {
    ...
    try {
      setGenerating(true)
      setError(null)
      const response = await fetch(`/api/crm/emails/${emailId}/draft?regenerate=true`, {
        method: 'POST'
      })
    ...
    try {
      ...
      const response = await fetch(`/api/crm/emails/${emailId}/reply`, {
        ...
        body: JSON.stringify({
          replyText: textValue,
          replyHtml: htmlValue,
          closeTicket: opts?.closeTicket || false,
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
              ...
              {threadMessages.length > 0 ? (
                  ...
                  >
                    <div className="flex justify-between items-start gap-4 mb-3 min-w-0">
                      <div className="flex items-center gap-3 flex-1 min-w-0 overflow-hidden">
                        {/* Clickable Avatar for Customer Info */}
                        {onToggleCustomer && (
                          <button
                            onClick={() => onToggleCustomer(msg.from)}
                        ...
                        isHtmlContent(msg.body) ? (
                          <div 
                            className="prose prose-sm max-w-none dark:prose-invert
                              prose-headings:font-semibold prose-headings:text-foreground prose-headings:mt-4 prose-headings:mb-3
                              prose-h1:text-2xl prose-h2:text-xl prose-h3:text-lg prose-h4:text-base
                              prose-p:text-foreground prose-p:leading-relaxed prose-p:my-3
                              prose-a:text-primary prose-a:no-underline hover:prose-a:underline prose-a:break-all prose-a:font-semibold
                              prose-strong:text-foreground prose-strong:font-bold
                              prose-em:italic prose-em:text-foreground
                              prose-ul:text-foreground prose-ul:my-3 prose-ul:pl-5
                              prose-ol:text-foreground prose-ol:my-3 prose-ol:pl-5
                              prose-li:text-foreground prose-li:leading-relaxed prose-li:my-1.5
                              prose-code:text-orange-600 dark:prose-code:text-orange-400 prose-code:bg-muted prose-code:px-2 prose-code:py-1 prose-code:rounded prose-code:text-sm prose-code:font-mono
                              prose-pre:bg-muted prose-pre:text-foreground prose-pre:p-4 prose-pre:rounded-lg prose-pre:overflow-x-auto prose-pre:my-3 prose-pre:border prose-pre:border-border
                              prose-blockquote:text-muted-foreground prose-blockquote:border-l-4 prose-blockquote:border-primary prose-blockquote:pl-4 prose-blockquote:italic prose-blockquote:my-3 prose-blockquote:bg-muted/30 prose-blockquote:py-2 prose-blockquote:rounded
                              prose-img:rounded-lg prose-img:max-w-full prose-img:h-auto prose-img:my-3 prose-img:shadow-md
                              prose-table:my-3 prose-table:w-full prose-table:border-collapse prose-th:bg-muted prose-th:text-foreground prose-th:font-semibold prose-th:p-3 prose-th:text-left prose-th:border prose-th:border-border
                              prose-td:border prose-td:border-border prose-td:p-3 prose-td:text-foreground
                              [&_*]:!text-foreground [&_*]:!color-inherit
                              break-words whitespace-pre-wrap bg-card/50 p-3 rounded-lg border border-border/30"
                            dangerouslySetInnerHTML={{ __html: sanitizeEmailHtml(msg.body) }}
                          />
                        ) : (
                          <div className="whitespace-pre-wrap break-words text-foreground leading-relaxed text-sm">
                            {msg.body}
                          </div>
                        )
                      ) : (
                ...
                emailSummary ? (
                  <div className="pb-4 border-b border-border/50 last:border-b-0">
                    ...
                    <div className="text-sm text-foreground/90 leading-relaxed overflow-hidden break-words">
                      {emailSummary.body ? (
                        isHtmlContent(emailSummary.body) ? (
                          <div 
                            className="prose prose-sm max-w-none 
                              ...
                              prose-pre:bg-muted prose-pre:text-foreground prose-pre:p-3 prose-pre:rounded prose-pre:overflow-x-auto prose-pre:my-2
                              prose-blockquote:text-muted-foreground prose-blockquote:border-l-4 prose-blockquote:border-primary prose-blockquote:pl-3 prose-blockquote:italic prose-blockquote:my-2
                              prose-img:rounded-lg prose-img:max-w-full prose-img:h-auto prose-img:my-2
                              prose-table:my-2 prose-table:w-full prose-th:bg-muted prose-th:text-foreground prose-th:font-semibold prose-th:p-2 prose-th:text-left
                              prose-td:border prose-td:border-border prose-td:p-2
                              break-words"
                            dangerouslySetInnerHTML={{ __html: sanitizeEmailHtml(emailSummary.body) }}
                          />
                        ) : (
                      ...
                      ) : emailSummary.snippet ? (
                        <div className="whitespace-pre-wrap break-words text-foreground/80 italic">
                          {emailSummary.snippet}
                        </div>
                      ) : (
                        <div className="text-muted-foreground italic">Loading content...</div>
                      )}
                    </div>
                  </div>
                ) : (
        ...
        <Card className="mx-4 md:mx-6 mb-4 shadow-lg">
          <div className="p-6 space-y-4">
            {onToggleCustomer && emailSummary && (
              <Button
                variant={showCustomerPanel ? "default" : "outline"}
                size="sm"
                onClick={() => onToggleCustomer(emailSummary.from)}
                className="w-full gap-2"
              >
                <User className="w-4 h-4" />
                {showCustomerPanel ? "Hide" : "Show"} Customer Info
              </Button>
            )}
            <Button
